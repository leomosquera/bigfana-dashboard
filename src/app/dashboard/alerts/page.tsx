...
import {
  Bell, TrendingUp, TrendingDown, AlertTriangle, CheckCircle, Info,
  Zap, Users, DollarSign, X,
} from "lucide-react";
...
const alertsData: {
  ...
  { id: 1, severity: "success",  title: "Revenue superó meta mensual",   description: "El revenue de Mayo 2026 alcanzó $2.84M, superando la meta de $2.5M en un 13.6%.",               time: "hace 12 min", category: "Revenue",     read: false },
  { id: 2, severity: "critical", title: "1,150 fans en riesgo de churn", description: "El modelo predictivo detectó un grupo de fans sin actividad > 60 días con alta probabilidad de abandono.", time: "hace 1h",     category: "Retención",   read: false },
  { id: 3, severity: "warning",  title: "Contrato Pepsi vence en 30 días", description: "El acuerdo de sponsoreo con Pepsi Co. vence el 31 de agosto. Se recomienda iniciar negociación.", time: "hace 3h",     category: "Sponsors",    read: false },
  { id: 4, severity: "success",  title: "Campaña Nike · Meta alcanzada", description: "La campaña Champions League Q3 superó 48M de impresiones. Conversión: 4.8%.",                   time: "hace 5h",     category: "Campañas",    read: true  },
  { id: 5, severity: "info",     title: "1,200 nuevos fans registrados hoy", description: "El registro diario superó el promedio mensual. Pico de ingreso entre 18h y 21h.",             time: "hace 6h",     category: "Fans",        read: true  },
  { id: 6, severity: "warning",  title: "Engagement bajó 4% esta semana",  description: "La semana sin partidos impactó el engagement. Se recomienda activar contenido alternativo.",   time: "hace 1d",     category: "Engagement",  read: true  },
  { id: 7, severity: "info",     title: "Nuevo récord: 890 upgrades a VIP", description: "La campaña de upgrade de temporada superó el récord previo con 890 nuevos fans VIP este mes.", time: "hace 2d",     category: "Gamificación", read: true },
];

const severityConfig: Record<AlertSeverity, {
  icon:       React.ComponentType<{ size?: number; className?: string }>;
  badgeVariant: "brand" | "success" | "warning" | "info";
  iconClass:  string;
  bg:         string;
  border:     string;
  label:      string;
}> = {
  critical: { icon: AlertTriangle, badgeVariant: "brand",   iconClass: "text-[#FF2D55]", bg: "bg-[#FF2D55]/10",  border: "border-[#FF2D55]/20",  label: "Crítico"  },
  warning:  { icon: TrendingDown,  badgeVariant: "warning", iconClass: "text-amber-400", bg: "bg-amber-500/10",  border: "border-amber-500/20",  label: "Atención" },
  success:  { icon: CheckCircle,   badgeVariant: "success", iconClass: "text-[#00D4A8]", bg: "bg-[#00D4A8]/10",  border: "border-[#00D4A8]/20",  label: "Positivo" },
  info:     { icon: Info,          badgeVariant: "info",    iconClass: "text-blue-400",  bg: "bg-blue-500/10",   border: "border-blue-500/20",   label: "Info"     },
};
...
export default function AlertsPage() {
  return (
    <PageShell
      ...
      actions={
        <>
          <Button intent="secondary" size="sm">Marcar todo como leído</Button>
          <Button intent="primary"   size="sm">Configurar alertas</Button>
        </>
      }
    >
      {/* Summary cards */}
              ...
              {alertTypes.map((item) => (
                <div key={item.label} className="flex items-center justify-between">
                  <div className="flex items-center gap-2.5">
                    ...
                    <span className={`text-xs ${item.active ? "text-[#F0F0F8]" : "text-[#55556A]"}`}>
                      {item.label}
                    </span>
                  </div>
                  <Switch defaultChecked={item.active} size="sm" />
                </div>
              ))}
            </div>
          </div>
